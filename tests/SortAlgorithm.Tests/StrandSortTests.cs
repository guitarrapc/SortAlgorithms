...
[InheritsTests]
public class StrandSortTests : StableSortTestsBase
{
    protected override void Sort<T, TContext>(Span<T> span, TContext context)
        => StrandSort.Sort(span, context);

    // O(n²) worst-case strand extraction makes large inputs slow.
    protected override int MaxOrderTestSize => 512;

    // Sorted input still performs the 3 buffer write phases (s→remaining, extraction→strand, merge→s).
    protected override CountExpectation SortedInputWrites => CountExpectation.NonZero;
    // StrandSort moves elements via buffer writes, never swaps.
