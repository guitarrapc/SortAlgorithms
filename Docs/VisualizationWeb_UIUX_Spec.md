...
|---------|------|-----------|---------------|
| 🎯 Sort Setup | 何をどう実行するか | Algorithm（+ ? リンク + Complexity）, Array Size, Add & Generate, Count | ❌ |
| 🎨 Appearance | どう見せるか | Scramble Pattern, Visualization Mode | ❌ |
...
| 🎯 Sort Setup | Algorithm（ドロップダウン + ? リンク + Complexity）、Array Size、Add & Generate（Upsert）、Count |
| 🎨 Appearance | Scramble Pattern、Visualization Mode |
...
| デバイス | 表示方式 |
|----------|----------|
| PC | 左サイドバー（常時表示、幅 280px 固定） |
| タブレット | 上段（常時表示、UI Cards 縦積み） |
| スマホ | 上段（常時表示、UI Cards 縦積み） |
...
| Array Size / Pattern | QAP: select | ❌ | 🎯 Sort Setup / 🎨 Appearance | ✕ メイン専用 |
...
│   ├── 🎯 SortSetupCard（Algorithm + Array Size + Add & Generate + Count）
│   ├── 🎨 AppearanceCard（Scramble Pattern + Visualization Mode）
│   └── ⚡ SpeedCard
