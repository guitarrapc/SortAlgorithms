...
|---------|------|-----------|---------------|
| 🎯 Sort Target | 何をソートするか | Algorithm（+ ? リンク + Complexity）, Array Size, Scramble Pattern | Algorithm のみ |
...
| 🚀 Action | 実行する | Add & Generate, Count | ❌（アルゴリズム選択時に自動ロード） |
...
| 🎯 Sort Target | Algorithm（ドロップダウン + ? リンク + Complexity）、Array Size、Scramble Pattern |
| 🎨 Display | Visualization Mode |
| ⚡ Speed | Operations Per Frame（スライダー）、Speed Multiplier（スライダー + FPS） |
| 🚀 Action | Add & Generate（Upsert）、Count |
...
| Complexity | QAP Sort Target 行内 / SortCard badge | 説明パネル badge | 🎯 Sort Target | △ 情報同一、表示形式が異なる |
