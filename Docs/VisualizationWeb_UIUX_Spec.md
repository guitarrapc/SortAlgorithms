...
現在の `ComparisonGridItem`（ヘッダー + 可視化 + Stats Summary）のパターンを全モードの基本単位とする。
...
**メリット:**
- 単一モード → Sort Card 1枚がフルサイズで表示
- 比較モード → Sort Card N枚がグリッドで表示
...
| シークバー | 単一モード時のみ、Sort Card の下に表示 |
| ステータス | 比較モード時の完了数表示（再生コントロール横） |
...
| Comparison Mode | トグルスイッチ + Add ボタン |
| Generate & Sort | プライマリボタン |
...
- **比較モード時**: 再生コントロール横に `"2/4 completed"` ステータスを表示
...
└────────────────────────────────────────────────────────┘
 ...
 ☰ タップ → 左からオーバーレイ
   (Algorithm, Size, Pattern, Mode, Speed, Ops/frame,
    Comparison ON/OFF, Generate & Sort)
```
...
|--------|------|
| 単一モード | Sort Card の下 |
| 比較モード | 非表示（各アルゴリズムの操作数が異なるため共通シークバーは不適） |
...
Comparison Mode トグルが ON の場合、以下が追加表示される:

```
│ Comparison Mode  [ON ──○]  │
│                            │
│ [➕ Add to Comparison]      │  ← 選択中のアルゴリズムを追加
│ (3 / 9 algorithms)        │
```

---

## 8. Settings モーダル（Layer 3）
...
| デバイス | 表示方式 | サイズ |
|----------|---------|--------|
| PC | 中央モーダル + 背景暗転 | max-width: 480px |
| タブレット | 中央モーダル + 背景暗転 | max-width: 480px |
| スマホ | フルスクリーンモーダル | 100vw × 100vh |
...
| セクション | 項目 | UIコントロール | 説明 |
|-----------|------|--------------|------|
| 🎨 Rendering | Renderer | セグメントコントロール (Canvas 2D / WebGL) | 描画エンジン切替。WebGL は高速だが互換性に注意 |
| ▶ Playback | Auto Reset on Complete | トグルスイッチ | ソート完了時に自動で初期状態に戻す |
| ▶ Playback | Instant Mode | トグルスイッチ | アニメーション無しで結果を即座に表示 |
| 🔊 Sound | Volume | スライダー (0%〜100%) | サウンド音量調整 |
| 🔧 Advanced | Auto Switch to Recommended Size | トグルスイッチ | アルゴリズム変更時に推奨サイズへ自動変更 |
| 🔧 Advanced | Category Filter | ドロップダウン | アルゴリズム一覧のカテゴリフィルタ |
| 🔧 Advanced | Debug Log | トグルスイッチ | コンソールにデバッグログを出力 |
...
比較モードでは複数アルゴリズムを同時実行し、Sort Card をグリッド表示する。
...
比較モード時はシークバーを非表示にする。各アルゴリズムの操作数が異なるため、共通のシークバーは意味をなさない。Play/Stop で一斉制御する。
...
| `compare` | bool | 比較モード有効 | `true` | (未指定=false) |
...
**比較モード:**
```
/?algo=QuickSort&size=512&pattern=Random&mode=BarChart&compare=true
```
...
状態変更時:
  1. ユーザーが Algorithm / Size / Pattern / Mode / Compare を変更
...
| Comparison Mode | 2 | Query String | ✅ | ✅ | URL次第 |
