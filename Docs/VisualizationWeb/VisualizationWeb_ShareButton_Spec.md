...
SortVivoのビジュアライゼーション画面でソートアルゴリズムの設定状態や比較状態を他者と共有するためのシェア機能を提供します。
...
- **パラメータ**:
  - `text`: シェアするテキスト（例: "SortVivoでソートアルゴリズムを可視化しています！"）
...
- **パラメータ**:
  - `url`: 現在のページURL（エンコード済み）
  - `title`: 投稿タイトル（例: "SortVivoでソートアルゴリズムを可視化！"）
- **動作**: 新しいウィンドウまたはタブを開く
...
現在のヘッダー構成:
```
[左空白] [SORTVIVO] [チュートリアルボタン][設定ボタン]
```

新しい構成:
```
[左空白] [SORTVIVO] [シェアボタン][チュートリアルボタン][設定ボタン]
```
...
- **スタイル**:
  ...
  - ボーダー: ゴールドアクセント（SortVivoのテーマカラー）
  - 角丸: 8px
  - ドロップシャドウ: あり
...
```
SortVivoでソートアルゴリズムを可視化！
```

多言語対応（ローカライゼーション）:
- 英語: "Visualizing sorting algorithms with SortVivo!"
- 日本語: "SortVivoでソートアルゴリズムを可視化！"
...
- `share.defaultText`: "SortVivoでソートアルゴリズムを可視化！"
