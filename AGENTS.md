...
- A Performance Characteristics row states an order, not a count this implementation happens to reach. Operation rows are as much a part of the theory as the running time — merge sort moves Θ(n log n) elements and bubble sort makes n(n-1)/2 swaps by definition — so a row that omits work the algorithm genuinely performs is wrong, not merely imprecise. Use the established labels so rows stay comparable across algorithms: `Family`, `Stable`, `In-place`, `Best case`, `Average case`, `Worst case`, `Comparisons`, `Swaps`, `Index Reads`, `Index Writes`, `Space`. Pad each to twelve characters before the colon.
