...
public static class TimSort
{
    ...
    /// </summary>
    /// <remarks>Verified by TimSortTests, which derives from StableSortTestsBase.</remarks>
        ...
        where TContext : ISortContext
    {
        ...
        // Detect natural run first, then sort from run end (matching reference implementations)
        if (n < MIN_MERGE)
        {
            var sortSpan = new SortSpan<T, TComparer, TContext>(span, context, comparer, BUFFER_MAIN);
            var initRunLen = CountRunAndMakeAscending(sortSpan, first, last);
            BinaryInsertionSort.SortCore(sortSpan, first, last, first + initRunLen);
            return;
        }
        ...
        where TContext : ISortContext
    {
        ...
        // Fixed-size run stack. 64 entries is ample for any int-indexed Span<T>.
        Span<int> runBase = stackalloc int[64];
    ...
    /// <summary>
    /// Counts the length of the run beginning at the specified position and makes it ascending.
    /// A run is the longest ascending sequence (a[lo] &lt;= a[lo+1] &lt;= ...)
    /// or the longest strictly descending sequence (a[lo] &gt; a[lo+1] &gt; ...).
    /// Strictly descending runs are reversed to become ascending.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int CountRunAndMakeAscending<T, TComparer, TContext>(SortSpan<T, TComparer, TContext> s, int lo, int hi)
        where TComparer : IComparer<T>
        where TContext : ISortContext
    {
        var runHi = lo + 1;
        ...
        where TContext : ISortContext
    {
        while (stackSize > 1)
        {
            ...
            // Enforce the corrected TimSort run-stack invariants using the
            // post-bugfix collapse strategy used in modern TimSort implementations.
            // Reference: "On the Worst-Case Complexity of TimSort" https://arxiv.org/abs/1805.08612
        ...
        where TContext : ISortContext
    {
        ...
        // Signal merge phase for visualization tracking
    ...
    private static void MergeLow<T, TComparer, TContext>(SortSpan<T, TComparer, TContext> s, int base1, int len1, int base2, int len2, ref T[]? tmpBuffer, ref int minGallop)
        ...
        where TContext : ISortContext
    {
        // Handle degenerate cases without temp buffer (C++ rotateLeft/rotateRight)
        if (len1 == 1)
        {
            var tmp = s.Read(base1);
            s.CopyTo(base2, s, base1, len2);
            s.Write(base1 + len2, tmp);
            return;
        }
        if (len2 == 1)
        {
            var tmp = s.Read(base2);
            s.CopyTo(base1, s, base1 + 1, len1);
            s.Write(base1, tmp);
            return;
        }
    ...
    exitMerge:
        ...
        if (len1 == 1)
        {
            // Last element of run1 is known to be > all remaining run2 elements
        ...
        {
            // len2 == 0: Run2 is exhausted, copy remaining run1 from temp
    ...
    {
        // Handle degenerate cases without temp buffer (C++ rotateLeft/rotateRight)
        // Note: len1 == 1 is unreachable here (MergeHigh requires len1 > len2 >= 1),
        // but kept for symmetry with MergeLow and the C++ reference (gfx::TimSort).
        if (len1 == 1)
        {
            var tmp = s.Read(base1);
            s.CopyTo(base2, s, base1, len2);
    ...
    exitMerge:
        ...
        if (len2 == 1)
        {
            // First element of run2 is known to be < all remaining run1 elements
            dest -= len1;
            cursor1 -= len1;
        ...
        {
            // len1 == 0: Run1 is exhausted, copy remaining run2 from temp
