...
public static class HeapSort
{
    ...
    private readonly struct HeapSortAction<T, TComparer> : ContextDispatcher.SortAction<T, TComparer>
        where TComparer : IComparer<T>
    {
        private readonly int _first;
        private readonly int _last;

        public HeapSortAction(int first, int last)
        {
            _first = first;
            _last = last;
        }
    ...
    public static void Sort<T>(Span<T> span, ISortContext context) where T : IComparable<T>
        => ContextDispatcher.DispatchSort(span, new ComparableComparer<T>(), context, new HeapSortAction<T, ComparableComparer<T>>(0, span.Length));
    ...
    public static void Sort<T>(Span<T> span, int first, int last, ISortContext context) where T : IComparable<T>
        => ContextDispatcher.DispatchSort(span, new ComparableComparer<T>(), context, new HeapSortAction<T, ComparableComparer<T>>(first, last));
    ...
    public static void Sort<T, TComparer>(Span<T> span, int first, int last, TComparer comparer, ISortContext context)
        where TComparer : IComparer<T>
        => ContextDispatcher.DispatchSort(span, comparer, context, new HeapSortAction<T, TComparer>(first, last));
