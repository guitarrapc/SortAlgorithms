...
public static class StableQuickSort
{
    ...
    private readonly struct StableQuickSortAction<T, TComparer> : ContextDispatcher.SortAction<T, TComparer>
        where TComparer : IComparer<T>
    {
        private readonly int _first;
        private readonly int _last;

        public StableQuickSortAction(int first, int last)
        {
            _first = first;
            _last = last;
        }
    ...
    public static void Sort<T>(Span<T> span, ISortContext context) where T : IComparable<T>
        => ContextDispatcher.DispatchSort(span, new ComparableComparer<T>(), context, new StableQuickSortAction<T, ComparableComparer<T>>(0, span.Length));
    ...
    public static void Sort<T>(Span<T> span, int first, int last, ISortContext context) where T : IComparable<T>
        => ContextDispatcher.DispatchSort(span, new ComparableComparer<T>(), context, new StableQuickSortAction<T, ComparableComparer<T>>(first, last));
    ...
    {
        ContextDispatcher.DispatchSort(span, comparer, context, new StableQuickSortAction<T, TComparer>(first, last));
    }
