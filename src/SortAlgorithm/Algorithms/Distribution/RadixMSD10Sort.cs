...
public static class RadixMSD10Sort
{
    ...
    private readonly struct RadixMSD10SortAction<T, TComparer> : ContextDispatcher.SortAction<T, TComparer>
    ...
    /// <summary>
    /// Sorts the elements in the specified span using MSD Radix Sort (base-10).
    ...
    /// <summary>
    /// Sorts the elements in the specified span using MSD Radix Sort (base-10) with sort context.
    /// </summary>
    public static void Sort<T>(Span<T> span, ISortContext context) where T : IBinaryInteger<T>, IMinMaxValue<T>
    {
        ContextDispatcher.DispatchSort(span, new ComparableComparer<T>(), context, new RadixMSD10SortAction<T, ComparableComparer<T>>());
    }
    ...
    public static void Sort<T, TComparer>(Span<T> span, TComparer comparer, ISortContext context)
        ...
        where TComparer : IComparer<T>
    {
        ContextDispatcher.DispatchSort(span, comparer, context, new RadixMSD10SortAction<T, TComparer>());
    }
